...
const ResultsReport: React.FC<Props> = ({
  ...
  // Actual test duration: prefer the value measured by the exercise itself,
  // fall back to the window markers, and only then to a 5-minute default.
  const metricsDuration = (metrics as any).testDurationSeconds;
  const windowDuration = ((window as any).exerciseEndTime - (window as any).exerciseStartTime) / 1000;
  const durationSeconds =
    (typeof metricsDuration === 'number' && isFinite(metricsDuration) && metricsDuration > 0) ? metricsDuration :
    (isFinite(windowDuration) && windowDuration > 0) ? windowDuration :
    300;
  const testDurationMinutes = Math.max(0.5, durationSeconds / 60);
  ...
  // Tiers aligned with SCORING v2 (strict top): 93+ is near-perfect typing.
  const getScoreLevel = (score: number) => {
    if (score >= 93) return { level: t.levels.professional, color: 'text-purple-600', bg: 'bg-purple-100' };
    if (score >= 78) return { level: t.levels.excellent, color: 'text-green-600', bg: 'bg-green-100' };
    if (score >= 60) return { level: t.levels.average, color: 'text-yellow-600', bg: 'bg-yellow-100' };
              ...
              <div className="space-y-2">
                {scoreBreakdown.breakdown.map((item, index) => (
                  <div key={index} className="flex justify-between items-center pb-1 border-b border-gray-200">
                    ...
                    {item.penalty < 0 ? (
                      <span className="text-green-600 font-bold text-sm">+{Math.abs(item.penalty)}</span>
                    ) : (
                ...
                <div className="mt-3 pt-2 border-t-2 border-gray-300">
                  ...
                  {scoreBreakdown.totalBonus > 0 && (
                    <div className="flex justify-between items-center text-green-600 text-sm">
                      <span className="font-semibold">{t.totalBonus || 'Speed Bonus'}:</span>
                      <span className="font-bold">+{scoreBreakdown.totalBonus}</span>
                    </div>
                  )}
                  ...
                  <p className="text-[10px] text-gray-400 mt-1 text-right">{t.scoreRangeNote || 'Final score is rounded and kept within the 45-100 range'}</p>
                </div>
              </div>
            </div>
          )}
