...
import { calculateOverallScore, calculateWastedTime, getScoreBreakdown } from './lib/scoring';

// Re-export so existing imports (ResultsReport, ShareCard) keep working.
// The single source of truth for scoring lives in src/lib/scoring.ts —
// the same module supabase.ts uses when saving, so the saved score
// always matches what the user saw on screen.
export { calculateOverallScore, calculateWastedTime, getScoreBreakdown };
...
function App() {
  ...
  const getTranslations = (key: string) => (t as any)[key] || {};
                ...
                <div>
                  ...
                  <div className="bg-blue-50 rounded-xl p-4 md:p-6 mb-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                      ...
                      <div className="text-center"><div className="text-xl md:text-2xl font-bold text-green-600">15%</div><div className="text-xs md:text-sm text-gray-700 font-medium">{t.beforeExercise.scoringCompletion}</div><div className="text-xs text-gray-500">{t.beforeExercise.scoringCompletionDesc}</div></div>
                      <div className="text-center"><div className="text-xl md:text-2xl font-bold text-purple-600">15%</div><div className="text-xs md:text-sm text-gray-700 font-medium">{t.beforeExercise.scoringSpeed}</div><div className="text-xs text-gray-500">{t.beforeExercise.scoringSpeedDesc}</div></div>
                      <div className="text-center"><div className="text-xl md:text-2xl font-bold text-orange-600">20%</div><div className="text-xs md:text-sm text-gray-700 font-medium">{t.beforeExercise.scoringFlow}</div><div className="text-xs text-gray-500">{t.beforeExercise.scoringFlowDesc}</div></div>
                    </div>
                  </div>
